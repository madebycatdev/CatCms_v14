/* inlinecancel-plugin.js is part of Aloha Editor project http://aloha-editor.org
 *
...
define([
	...
	function onSelect(selected, range) {
		...
		if ($swatch.hasClass('removecolor')) {
			inlinecancel.unsetColor(range);
		} else {
			inlinecancel.setColor(
				range,
	...
	function getOverlay(editable, plugin, button, getSwatchClass) {
		...
		if (!overlay) {
			...
			overlay.$element
			       .addClass('aloha-ui-inlinecancel-picker-overlay')
	...
	return Plugin.create('inlinecancel', {

		config: Palette,

		_constructor: function () {
			this._super('inlinecancel');
		},

		init: function () {
			var plugin = this;

			if (Aloha.settings.plugins && Aloha.settings.plugins.inlinecancel) {
				plugin.config = Aloha.settings.plugins.inlinecancel;
			}
			
			var button = Ui.adopt('colorPicker', Button, {
				tooltip: i18n.t('button-text'),
				icon: 'aloha-icon-inlinecancel',
